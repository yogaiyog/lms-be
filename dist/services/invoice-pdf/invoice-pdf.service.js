...
exports.InvoicePdfService = exports.invoicePdfInclude = void 0;
const pdf_lib_1 = require("pdf-lib");
const qrcode_1 = __importDefault(require("qrcode"));
const env_1 = require("../../config/env");
exports.invoicePdfInclude = {
    enrollment: {
        include: {
            student: {
                include: {
...
const COLORS = {
    primary: (0, pdf_lib_1.rgb)(0.13, 0.41, 0.63),
    dark: (0, pdf_lib_1.rgb)(0.09, 0.30, 0.47),
    accent: (0, pdf_lib_1.rgb)(0.07, 0.72, 0.42),
    warning: (0, pdf_lib_1.rgb)(0.92, 0.50, 0.07),
    danger: (0, pdf_lib_1.rgb)(0.85, 0.18, 0.14),
    text: (0, pdf_lib_1.rgb)(0.15, 0.15, 0.15),
    muted: (0, pdf_lib_1.rgb)(0.50, 0.50, 0.50),
    light: (0, pdf_lib_1.rgb)(0.95, 0.96, 0.98),
    softBlue: (0, pdf_lib_1.rgb)(0.94, 0.96, 1),
    white: (0, pdf_lib_1.rgb)(1, 1, 1),
    border: (0, pdf_lib_1.rgb)(0.87, 0.88, 0.90),
    rowAlt: (0, pdf_lib_1.rgb)(0.975, 0.98, 0.985),
};
function drawRect(page, x, y, w, h, color, border) {
    page.drawRectangle({
        x,
        y,
        width: w,
...
function safeText(text, font, size) {
    try {
...
function drawText(page, text, x, y, size, font, color = COLORS.text, options = {}) {
...
function badgeWidth(text, font, size, hPad = 10) {
    return font.widthOfTextAtSize(safeText(text, font, size), size) + hPad * 2;
}
function drawBadge(page, text, x, y, font, bg, fg = COLORS.white, size = 8, hPad = 10) {
...
function ewalletName(paymentType) {
    const map = {
        gopay: "GoPay",
        shopeepay: "ShopeePay",
        dana: "DANA",
...
class InvoicePdfService {
    async generatePdf(invoice) {
        ...
        doc.setAuthor(env_1.env.COMPANY_NAME);
        ...
        const drawSectionHeader = (title, color = COLORS.primary) => {
            ensureSpace(36);
            const h = 24;
        ...
        const drawInfoGrid = (items, cols = 2) => {
        ...
        const activePayment = invoice.payments
        ...
        drawText(page, env_1.env.COMPANY_NAME, MARGIN, PAGE_H - 46, 15, bold, COLORS.white);
        drawText(page, "INVOICE", MARGIN, PAGE_H - 84, 28, bold, COLORS.white);
        drawText(page, `No. ${invoice.number}`, MARGIN, PAGE_H - 104, 10, font, (0, pdf_lib_1.rgb)(0.85, 0.9, 0.97));
        const statusText = invoice.status;
        const statusColor = STATUS_COLORS[invoice.status] ?? COLORS.muted;
        drawText(page, "Diterbitkan", PAGE_W - MARGIN, PAGE_H - 44, 8, font, (0, pdf_lib_1.rgb)(0.8, 0.87, 0.96), { align: "right" });
        ...
        if (invoice.status === "PAID") {
            ...
            drawText(page, `Jumlah yang harus dibayar: ${formatIDR(Number(activePayment.amount))}`, MARGIN + 5, y, 10, bold, COLORS.text);
            y -= 24;
            if (activePayment.paymentType === "bank_transfer") {
                ...
                by -= 44;
                drawText(page, `Transfer ke nomor VA di atas melalui ${activePayment.bank?.toUpperCase() ?? "bank terkait"}. Pembayaran terverifikasi otomatis — simpan bukti transfer untuk berjaga-jaga.`, MARGIN + 16, by, 8.5, font, COLORS.muted, { maxWidth: CONTENT_W - 32 });
                ...
                const boxTop = y;
                drawRect(page, MARGIN, boxTop - boxH, CONTENT_W, boxH, COLORS.light, { color: COLORS.border, width: 1 });
                const qrBuffer = await qrcode_1.default.toBuffer(qrText ?? "QRIS", { type: "png", width: qrSize, margin: 1 });
                ...
                const boxTop = y;
                drawRect(page, MARGIN, boxTop - boxH, CONTENT_W, boxH, COLORS.light, { color: COLORS.border, width: 1 });
                const qrBuffer = await qrcode_1.default.toBuffer(activePayment.deeplinkUrl, { type: "png", width: qrSize, margin: 1 });
                const qrImage = await doc.embedPng(qrBuffer);
                const qrX = MARGIN + 16;
                const qrY = boxTop - boxH / 2 - qrSize / 2;
                drawRect(page, qrX, qrY, qrSize, qrSize, COLORS.white, { color: COLORS.border, width: 1 });
                ...
                ensureSpace(40);
                drawText(page, "Tidak ada detail pembayaran — silakan buat ulang metode pembayaran.", MARGIN + 5, y, 9, font, COLORS.text);
                y -= SPACE.lg;
            }
        }
        else {
            ...
            drawText(page, "Belum ada metode pembayaran dipilih. Admin dapat memilih metode via dashboard Keuangan.", MARGIN + 5, y, 9, font, COLORS.text, { maxWidth: CONTENT_W - 10 });
            y -= SPACE.lg;
        }
        // ══════════════════════════ RIWAYAT PEMBAYARAN ══════════════════════════
        ...
        pages.forEach((p, idx) => {
            ...
            drawText(p, `${env_1.env.COMPANY_NAME} — invoice ini berlaku sesuai syarat & ketentuan`, MARGIN, 28, 7.5, font, COLORS.muted);
            drawText(p, `Digenerate ${new Date().toLocaleString("id-ID")}`, MARGIN, 16, 7, font, COLORS.muted);
