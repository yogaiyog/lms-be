...
const COLORS = {
    ...
    light: (0, pdf_lib_1.rgb)(0.92, 0.92, 0.92),
    white: (0, pdf_lib_1.rgb)(1, 1, 1),
    border: (0, pdf_lib_1.rgb)(0.85, 0.85, 0.85),
};
...
function getGradeLabel(pct) {
    if (pct >= 90)
        return { label: "Excellent", color: (0, pdf_lib_1.rgb)(0.07, 0.72, 0.42) };
    if (pct >= 75)
        return { label: "Good", color: (0, pdf_lib_1.rgb)(0.13, 0.41, 0.63) };
    if (pct >= 60)
        return { label: "Cukup", color: COLORS.warning };
...
class ReportPdfService {
    async generatePdf(data) {
        ...
        const drawSectionHeader = (title) => {
            ensureSpace(35);
            drawRect(page, MARGIN, y - 22, CONTENT_W, 22, (0, pdf_lib_1.rgb)(0.95, 0.96, 0.98));
        ...
        const summaryItems = [
            ...
            { label: "Total Skor", value: `${data.totalScore} / ${data.maxScore}`, color: (0, pdf_lib_1.rgb)(0.13, 0.41, 0.63) },
        ...
        for (let i = 0; i < summaryItems.length; i++) {
            ...
            drawRect(page, cx, y - cardH, cardW, cardH, (0, pdf_lib_1.rgb)(0.96, 0.97, 0.98));
        ...
        for (const p of doc.getPages()) {
            drawRect(p, 0, 0, PAGE_W, 50, (0, pdf_lib_1.rgb)(0.95, 0.96, 0.98));
            ...
            drawText(p, `(c) ${env_1.env.COMPANY_NAME}`, PAGE_W / 2, 24, 7, font, COLORS.muted, { align: "center" });
        }
