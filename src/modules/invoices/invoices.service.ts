...
export function calculateInvoice(subtotal: number | Decimal, taxPercent: number | Decimal | null, registrationFee?: number | Decimal) {
...
export async function syncEnrollmentMeetCounts(tx: Tx, enrollmentId: string) {
...
export async function updateInvoiceStatus(tx: Tx, invoiceId: string) {
  ...
  const data: { status: string; paidAt?: Date | null } = { status: newStatus };
