...
reportsRouter.get("/schedule/:scheduleId", authenticate, async (req, res, next) => {
  try {
    ...
    const studentsReports: StudentReport[] = schedule.attendances
      ...
      .map((attendance) => {
        ...
        const aspectAnalysis: AspectAnalysis[] = assessment.scores.map((s) => {
    ...
    const classAverage = studentsReports.length > 0
      ? studentsReports.reduce((sum: number, r) => sum + r.assessment.percentage, 0) / studentsReports.length
      : 0;
...
reportsRouter.get("/student/:studentId/report-summary", authenticate, async (req, res, next) => {
  try {
    ...
    for (const att of attendances) {
      ...
      if (statusCounts[att.status as keyof typeof statusCounts] !== undefined) {
...
reportsRouter.get("/class/:classId/done-schedules", authenticate, async (req, res, next) => {
  try {
